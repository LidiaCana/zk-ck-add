...
const MY_CONTRACT_ADDRESS = "0x055C61F144d6BC9555F2ffa99142FEa90A0c6811";
  ...
  var awaitWeb3 = async function () {
    ...
    web3.eth.net.getId((err, netId) => {
      if (netId == NETWORK_ID) {
        ...
        document.getElementById("web3_message").textContent =
          "Please connect to Scroll Testnet";
      }
    });
  };
  awaitWeb3();
...
const sendProof = async (a, b) => {
  ...
  const { proof, publicSignals } = await snarkjs.groth16.fullProve(
    { a: a, b: b },
    "../zk_artifacts/add.wasm",
    "../zk_artifacts/add_final.zkey"
  );

  const vkey = await fetch("../zk_artifacts/verification_key.json").then(
    function (res) {
      return res.json();
    }
  );
  ...
  const result = await my_contract.methods
    .sendProof(pA, pB, pC, publicSignals)
